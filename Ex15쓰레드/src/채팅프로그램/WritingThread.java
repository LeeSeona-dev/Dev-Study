...
//채팅을 사용자가 작성하고 작성한 채팅을 -> 서버로 보내기
public class WritingThread  extends Thread{//멀티쓰레딩->사용자들이 동시에 채팅 가능하게!
	...
	public void run() { //클라이언트가 작성한 채팅을 서버로 보내기
		...
		try {
		  ...
		  while(true) {
			...
			//socket정보가 잘못됐을 경우에는 스트림이 정상적으로 생성되지 않을 수 있으므로 예외처리
			e.printStackTrace();
		}
	}
